...
public class EncryptDataDto
{
    ...
    public string Decrypt(string password)
    {
        ...
        return EncryptType switch
            {
                SecurityServer.Common.EncryptType.AesCbc => Encoding.UTF8.GetString(EncryptHelper.AesCbcDecrypt(cipherText, key, nonce)),
                SecurityServer.Common.EncryptType.AesGcm => Encoding.UTF8.GetString(EncryptHelper.AesGcmDecrypt(cipherText, key, nonce, tag)),
    ...
    public string Encrypt(string password, string secret)
    {
        ...
        {
            SecurityServer.Common.EncryptType.AesCbc => EncryptHelper
                .AesCbcEncrypt(Encoding.UTF8.GetBytes(secret), passwordByte, nonceByte).ToHex(),
            SecurityServer.Common.EncryptType.AesGcm => EncryptHelper
